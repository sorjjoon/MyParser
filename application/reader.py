    ...
    try:
        with open(path, "r", encoding="iso-8859-1") as f:
            ...
            for row in f:
                ...
                # saves time, read only rows while we are in a match (might be a lot of rows between matches)
                if in_match:
                    ...
                    # Safe Login immunity means a loading screen (a game has ended)
                        ...
                        if len(rounds) == 3:
                            matches.append(match(
                                start, end, rounds[0], rounds[1], rounds[2], team, opponents, match_number))
                        else:
                            matches.append(
                                match(start, end, rounds[0], rounds[1], None, team, opponents, match_number))
                        match_number += 1
                        in_match = False
                        rounds = []
                    ...
                    # {3289210509328384} is id for Bolster (notes the start of a game)
                    if "{3289210509328384}" in row:
                        row_object = read_row(row)
