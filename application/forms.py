...
def upload_log():
    if request.method == "POST" and "log" in request.files: 
        ...
        try:
            ...
            return render_template("upload.html", form = LogForm(), errors = ["Upload a valid file"])
        ...
        if not raw_matches:
            ...
            return render_template("upload.html", form = LogForm(), errors = ["Upload a valid file, either the uploaded file is empty, or the log you uploaded had no arena games in it"])
