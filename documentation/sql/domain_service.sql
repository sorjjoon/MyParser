...
INSERT INTO player (name, player_class) VALUES ('@something', 'Operative') 
...
SELECT DISTINCT log.id, log.start_date, log.note, char.name FROM log 
JOIN char ON log.char_id = char.id 
WHERE log.owner_id = 2 ORDER BY log.start_date DESC;  
...
SELECT * FROM log JOIN char ON char.id = log.char_id WHERE log.id = 2 AND log.owner_id = 2; 
--if only_details is true, get_matches (see line 101) function is used to make a list of all matches belonging to a log (otherwise an empty list is used)
...
-- The list of match ids belonging to the match is fetched with the function get_match_ids (see line 76), and each match_id is verified to be contained in the list
-- if any match_id isn't found in the list, the transaction is rolled back
-- match updating is done with the function update_match (see line 28)



--def insert_match(self, log_id: int, match: match):
INSERT INTO match (log_id, round1, round2, round3, start_time, end_time, note) VALUES(2, false, true, true, '10:05:06.789', '10:20:05.123', 'something')


--def get_match_ids(self, log_ids):
SELECT id FROM match WHERE log_id IN (1,2,3);
...
SELECT match_player.player_id, match_player.match_id, match_player.owner_side, player.name FROM match_player JOIN player ON player.id = match_player.player_id WHERE match.match_id = 2
...
SELECT char.id FROM char WHERE name = '@something' AND owner_id = 2;
--if not found, insert a new char
INSERT INTO char (name, owner_id) VALUES ('@something', 2); 
--after everything, a new log is inserted the char.id of the foung/inserted char
INSERT INTO log (owner_id, start_date, char_id) VALUES (2, '2019-07-01', 2);
--matches are insrted using the function insert_match (see line 72)


--def get_matches(self, log_ids):
SELECT * FROM match WHERE log_id IN (1,2,3,4);
--the team/opponents of each match is done using the function get_team_and_opponents  (see line 81)
